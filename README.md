...
An `Instantiator` is not `Sendable` – if you want to instantiate a type on a different queue than the one used by your root object, use a [`SendableInstantiator`](Sources/SafeDI/DelayedInstantiation/SendableInstantiator.swift).
